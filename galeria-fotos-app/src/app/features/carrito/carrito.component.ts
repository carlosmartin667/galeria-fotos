...
import { Carrito, CarritoItem, CrearPedidoDesdeCarritoResponse } from '../../core/models/carrito.models';
...
@Component({
  ...
  standalone: true,
  imports: [CurrencyPipe, NgFor, NgIf, RouterLink, ReactiveFormsModule, EmptyStateComponent, ErrorAlertComponent, LoadingComponent],
...
export class CarritoComponent implements OnInit {
  ...
  get pedidoId(): string {
    return this.pedidoCreado?.pedido?.id || this.pedidoCreado?.pedidoId || this.pedidoCreado?.id || '';
  }
  ...
  load(clearMessages = true): void {
    ...
    this.carritoService.getCarrito().pipe(
      finalize(() => {
        this.loading = false;
  ...
  aplicarCupon(): void {
    ...
    this.applying = true;
    this.carritoService.aplicarCupon(this.couponForm.controls.codigo.value.trim()).pipe(
      finalize(() => {
        this.applying = false;
        this.cdr.markForCheck();
      })
  ...
  quitarCupon(): void {
    ...
    this.couponError = '';
    this.success = '';
    this.carritoService.quitarCupon().pipe(
      finalize(() => {
  ...
  quitarItem(item: CarritoItem): void {
    ...
    this.error = '';
    this.carritoService.quitarItem(id).pipe(
      finalize(() => {
        this.actionItemId = '';
        this.cdr.markForCheck();
      })
  ...
  crearPedido(): void {
    ...
    this.carritoService.crearPedido().pipe(
      finalize(() => {
      ...
      error: (error: unknown) => {
        this.error = error instanceof Error ? error.message : 'No se pudo crear el pedido desde el carrito.';
      }
    });
  }
  ...
  private couponMessage(error: unknown): string {
    ...
    if (normalized.includes('invalid') || normalized.includes('invalido') || normalized.includes('no existe')) {
