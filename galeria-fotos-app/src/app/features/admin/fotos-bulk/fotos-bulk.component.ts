...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, NgFor, NgIf, ReactiveFormsModule, ErrorAlertComponent, LoadingComponent],
...
export class FotosBulkComponent implements OnInit {
  ...
  get nombresArchivo(): string[] {
    return this.form.controls.nombresArchivo.getRawValue()
      .split(/[\n,;]+/)
      .map((item) => item.trim())
      .filter(Boolean);
  }
  ...
  loadEventos(): void {
    ...
    this.eventosService.list().pipe(
      ...
      error: (error: unknown) => {
        this.error = error instanceof Error ? error.message : 'No se pudieron cargar los eventos.';
      }
    });
  }
  ...
  generarStorageKeys(): void {
    ...
    this.fotosService.generarStorageKeysBulk({ eventoId, nombresArchivo }).subscribe({
      ...
      error: (error: unknown) => {
        this.error = error instanceof Error ? error.message : 'No se pudieron generar las storage keys.';
        this.generating = false;
        this.cdr.markForCheck();
      }
    });
  ...
  crearMetadataBulk(): void {
    ...
    const fotos = this.generatedItems
      .map((item, index) => ({
        ...
        previewUrl: this.previewUrl(raw.previewBaseUrl, this.fileNameOf(item) || this.nombresArchivo[index] || ''),
  ...
  responseValue(response: GenerarStorageKeysBulkResponse | CrearFotoMetadataBulkResponse | null, key: string): string | number {
    if (!response) {
      return '-';
    }

    const matchingKey = Object.keys(response).find((item) => item.toLowerCase() === key.toLowerCase());
    const value = matchingKey ? response[matchingKey] : undefined;
    return typeof value === 'number' || typeof value === 'string' ? value : '-';
  }

  errores(response: GenerarStorageKeysBulkResponse | CrearFotoMetadataBulkResponse | null): unknown[] {
    if (!response) {
      return [];
    }
  ...
  private normalizeStorageKeys(response: GenerarStorageKeysBulkResponse, nombresArchivo: string[]): StorageKeyBulkItem[] {
    ...
    const arrayKey = ['items', 'Items', 'resultados', 'Resultados', 'storageKeys', 'StorageKeys', 'data', 'Data', 'results', 'Results']
      .find((key) => Array.isArray(record[key]));

    if (arrayKey) {
      return (record[arrayKey] as unknown[]).filter((item): item is StorageKeyBulkItem => !!item && typeof item === 'object');
    }

    const mapValue = record['storageKeys'] ?? record['StorageKeys'];
    if (mapValue && typeof mapValue === 'object' && !Array.isArray(mapValue)) {
      return Object.entries(mapValue as Record<string, unknown>).map(([nombreArchivo, storageKey]) => ({ nombreArchivo, storageKey: String(storageKey ?? '') }));
    }
  ...
  private previewUrl(base: string, nombreArchivo: string): string | undefined {
    ...
    return cleanBase && nombreArchivo ? `${cleanBase}/${encodeURIComponent(nombreArchivo)}` : undefined;
  }
}
